...
if __name__ == '__main__':
    ...
    pointnum = int(4501/stepsize) #how many datapoints we are collecting
    finaloutput = np.zeros((pointnum,9)) #the array that will be saved to file containing all of the data
    
    #begin looping over earthquake input sizes
    countkk = -1
    for kk in range(stepsize, 4501, stepsize):
        countkk+=1
        print(kk)
        resize = kk
        ...
        #prepare the dataset
        with open('/home/davidgrzan/Tsunami/cascadia/machinelearning/largeuplift3.txt', "r") as csvfile:
        ...
        difffinal = float(totalavgdiff/testnumber)
        confinederrorfinal = float(totalavgerror/testnumber)
        L1errorfinal = float(totalavgL1error/testnumber)

        finaloutput[countkk] = np.array([layersize, resize, epochloss, myloss, L1errorfinal, confinederrorfinal, L1sdnum, confinedsdnum, difffinal])
        print(finaloutput)

    np.savetxt("/home/davidgrzan/Tsunami/cascadia/machinelearning/data800nodes.txt", finaloutput, delimiter=",", fmt='%f')

        
    #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    """
    out_dataset = Dataset("/home/davidgrzan/Tsunami/cascadia/machinelearning/torchout8.nc", 'w', format='NETCDF4')
